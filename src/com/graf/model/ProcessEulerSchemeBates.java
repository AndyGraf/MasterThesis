...
import Modelandprocess.AbstractProcessBates;
 ...
 * @version 1.4
 */
public class ProcessEulerSchemeBates extends AbstractProcessBates {
	
	private static boolean isUseMultiThreadding;
	static {
		// Default value is true
		isUseMultiThreadding = Boolean.parseBoolean(System.getProperty("net.finmath.montecarlo.process.ProcessEulerScheme.isUseMultiThreadding","false"));
	}
	 ...
	 */
	public ProcessEulerSchemeBates(IndependentIncrementsInterface stochasticDriver, Scheme scheme) {
	 ...
	 * @param stochasticDriver The stochastic driver of the process (e.g. a Brownian motion).
	 */
	public ProcessEulerSchemeBates(IndependentIncrementsInterface stochasticDriver) {
		super(stochasticDriver.getTimeDiscretization());
		this.stochasticDriver = stochasticDriver;
	}
			...
			for (int componentIndex2 = 0; componentIndex2 < numberOfComponents; componentIndex2++) {
				...
				Callable<RandomVariableInterface> worker = new  Callable<RandomVariableInterface>() {
					public RandomVariableInterface call() throws SolverException {
						...
						// Add increment of jump component
						increment =increment.add(getJumpComponent(timeIndex-1, discreteProcess[timeIndex - 1]));
	...
	@Override
	public ProcessEulerSchemeBates clone() {
		return new ProcessEulerSchemeBates(getStochasticDriver(), scheme);
	}

	@Override
	public AbstractProcessInterfaceBates getCloneWithModifiedData(Map<String, Object> dataModified) {
	...
	public Object getCloneWithModifiedSeed(int seed) {
		return new ProcessEulerSchemeBates(getBrownianMotion().getCloneWithModifiedSeed(seed));
	}
